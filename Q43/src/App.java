...
class Fibonacci extends Thread{
    ...
    public void run(){
        Thread t = Thread.currentThread();t.setName("Fibonacci");
...
class Prime extends Thread
{
    ...
    public void run(){
        Thread t= Thread.currentThread();t.setName("Prime");
        System.out.println(t.getName() + " thread Started...");
        int prime=1;
        while(true){
            try{
                if(prime>100000){out1.close();break;}
                if(isPrime(prime))
                out1.write(prime);prime++;sleep(0);
            }
            catch(Exception e){
                System.out.println(t.getName() + " thread exiting.");
                System.exit(0);
...
public class App{
    public static void main(String[] args) throws Exception    {
        Thread t=Thread.currentThread();t.setName("Main");
        System.out.println(t.getName() + " thread Started...");
        Fibonacci fibonacci = new Fibonacci();Prime prime = new Prime();
        PipedReader fpr = new PipedReader(fibonacci.getPipedWriter());
        PipedReader ppr = new PipedReader(prime.getPipedWriter());
        fibonacci.start();prime.start();
